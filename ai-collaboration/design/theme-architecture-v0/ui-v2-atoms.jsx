 ...
 */
function AnyuPage({ children, bleeds = [], grain = true, tokens = null }) {
  ...
  // Module Theme Pack swap: inline custom props on THIS element win over the
  // .anyu-v2 class declaration on the same element (which re-sets --anyu-accent).
  // So bleeds + every descendant atom re-skin from these hexes.
  const tv = {};
  if (tokens) {
    const a = (hex, al) => { const m = hex.replace('#',''); return `rgba(${parseInt(m.slice(0,2),16)},${parseInt(m.slice(2,4),16)},${parseInt(m.slice(4,6),16)},${al})`; };
    if (tokens.accent)  { tv['--anyu-accent'] = tokens.accent; tv['--anyu-accent-12'] = a(tokens.accent, .12); tv['--anyu-accent-20'] = a(tokens.accent, .22); tv['--anyu-accent-45'] = a(tokens.accent, .50); }
    if (tokens.accent2) { tv['--anyu-accent2'] = tokens.accent2; tv['--anyu-accent2-15'] = a(tokens.accent2, .18); tv['--anyu-accent2-30'] = a(tokens.accent2, .40); }
    if (tokens.rose)    { tv['--anyu-rose'] = tokens.rose; tv['--anyu-rose-20'] = a(tokens.rose, .25); }
  }
  return (
