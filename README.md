...
Staging push is part of the normal completion workflow.

Production deployment is not.

- Production deploy is never automatic after a handoff.
- Production deploy requires explicit human approval.
- Production deploy requires a separate launch decision record.
- Production env changes and production DB migration must be treated as manual operational steps.
