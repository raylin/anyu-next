...
describe("NewebPay checkout route", () => {
  beforeEach(() => {
    ...
    mockCreateNewebPayCheckout.mockResolvedValue({
      ...
      checkoutContract: {
        ...
        returnUrl:
          "https://staging.anyu.tw/m/ambiguous-temperature/payment/return?merchantOrderNo=ANYUNPORDEREXISTING000000000",
        fields: {
  ...
  it("creates a pending checkout contract without paid delivery state", async () => {
    ...
    expect(data).toEqual({
      ...
      pendingReturnPath:
        "/m/ambiguous-temperature/payment/return?merchantOrderNo=ANYUNPORDEREXISTING000000000",
    });
